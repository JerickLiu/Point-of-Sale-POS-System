...
class InventoryMenu(Inventory):
	...
	# Class InventoryMenu contains a list of all products as objects of the Inventory class
	...
	def inventoryMenu(self):
		...
		# Boolean variable tracking if user wants to go back to Main Menu is False
		exit = False
		...
		# Changes are updated to the productList in main
	...
	def addProduct(self):
		...
		# Boolean variables used to loop / confirm selection. 
		#   > addConfirm will ask user if they'd like to add the product. 
		#   > goodQuantity will continuously loop until the user inputs a quantity amount higher than the min. qty.
		#   > discountTest will continuously loop until the user inputs percentages that make the price higher than vendor price

		addConfirm, goodQuantity, discountTest = False, False, False
		...
		# While statement (see goodQuantity note above)
		while not goodQuantity:
			...
			# Calls quantityHigherThanMin method to determine if values are valid
			goodQuantity = self.quantityHigherThanMin(addQuantity, addMinQty)
		...
		# While statement (see discountTest note above)
		...
		if FindProduct.checkIfProductExists(self.productList, addName):
			...
			# Calls confirmSelection method and prints a formatted view of the new product (see addConfirm note above)
			addConfirm = Functions.confirmSelection(self.formattedAddProduct(addSKU, addName, addQuantity, addMinQty, addVendorPrice, addMarkUp, addSalePercent))

			# If statement checks if user confirmed the addition of the new product
			if addConfirm:
	...
	def applySale(self):
		# Applies a sale percentage to select products
		# Parameters: self (instance)
		# Returns: Null
		...
		if userChoice == '1':
			...
			if FindProduct.checkIfProductExists(self.productList, userProduct):
				...
				# While statement calls priceHigherThanVendor method and loops as long as the price is lower than the vendor (illegal)
	...
	def modifyProduct(self):
		...
		if FindProduct.checkIfProductExists(self.productList, userProduct):
			...
			if userChoice == '1':
				...
				# If statement calls checkIfNameExists method and checks if the name isn't taken (in the productList)
				if not FindProduct.checkIfNameExists(self.productList, newName):
			...
			# User chooses to modify vendor price
				...
				# Calls updateVendorPrice method to update respective object's attribute (no need to check if price is valid as all prices change in retrospect of the vendor price)
	...
	def checkIfNameExists(self, name):
		# Checks if a string is present in the productList
		# Parameters: self (instance), name (str)
		# Returns: True if string present / False if not (boolean)

		nameExists = False

		# For statement loops through all products
		for product in self.productList:

			# If statement checks if the object attribute name is equal to the string (not case-sensitive)
	...
	def reOrderStockMenu(self):
		...
		if userChoice == '1':
			...
			if FindProduct.checkIfProductExists(self.productList, userSKU):
				...
				# Calls reOrderSearchProduct method
		...
		# User chooses to restock by min. qty.
			...
			self.reOrderStockUnderQty(userQuantity, userAmount)
		...
		# User chooses to restock by warning indicator
			...
			self.reOrderWarningQty(userAmount)
	...
	def reOrderStockUnderQty(self, minQty, amount):
		...
		for product in self.productList:
			...
			if product.quantity < int(minQty) and product.quantity + amount > product.minQuantity:
	...
	def productReportMenu(self):
		...
		while not doneReporting:
			...
			# Creates Outputs Class to control printing of abndance of outputs
			outputManager = Outputs(0)
			...
			# User chooses to report based on max qty
				...
				Reports.criteriaQuantity(self.productList, userQuantity, outputManager)
	...
	@staticmethod
	def formattedAddProduct(sku, name, quantity, minQty, vendPrice, markUp, salePercent):
    # Creates a formatted product string for user visability
    # Parameters: sku (str), name (str), quantity (str), minQty (int), vendPrice (float), markUp (int), salePercent (int)
	...
	@staticmethod
	def priceHigherThanVendor(vendPrice, markUp, salePercent):
		# Checks if calculated current price will be lower than vendor price
		# Parameters: vendPrice (float), markUp (int), salePercent (int)
	...
	@staticmethod
	def quantityHigherThanMin(quantity, minQty):
		# Small method checking if quantity is greater than min. qty.
		# Parameters: quantity (int), minQty (int)
		# Returns: True if greater / False if less than (boolean)
		return True if quantity > minQty else False
	# end quantityHigherThanMin method


	@staticmethod
	def convertCategory(userInput):
...
class Reports:
	...
	def criteriaQuantity(productList, userQuantity, outputManager):
		# Prints all products under specified quantity
		# Parameters: productList (List), userQuantity (int), outputManager (Outputs object)
		# Return: Null
